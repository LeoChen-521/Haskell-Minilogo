...
cmd (Move e1 e2) (Down, (x, y)) = ((Down, (expr (e1), expr (e2))), Just ((x, y), (expr (e1), expr (e2))))
...
block (x:xs) s = case (cmd x s) of
                (nexts, Nothing)   -> block xs nexts
                (nexts, Just line) -> (fst(block xs nexts), [line] ++ snd(block xs nexts))
...
optimize (x:xs) = if (prettyCmd x) == "pen up" then [x] ++ optimize xs 
                 ...
                 else case optimizeCmd x of [a,b] -> [(Move (Lit (expr (a))) (Lit (expr (b))))] ++ optimize xs
                 
optimizeCmd :: Cmd -> [Expr]
optimizeCmd (Move e1 e2) = [e1, e2]
