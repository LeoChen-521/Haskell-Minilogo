...
data Reg = A | B
data Exp = Lit Int | R Reg | Add Exp Exp
data Stmt = Set Reg Exp | Swap | IF Reg Reg
type Prog = [Stmt]

example :: Stmt
example = If (Set A (Lit 2)) (Set B (Add (R A) (Lit 3))) then Swap else Set B (Add (R A) (R B))

data Type = TInt | TReg | TStmt
